...
public class FooProcedureWorld2d extends ProcedureWorld2d<FooProcedureObject2d, FooProcedureWorld2d, FooProcedureDrawingOptions> {
	...
	private FooCard2d lastFlippedCard = null;
	
	public void registerFlippedCard(FooCard2d card2d) {
	...
	public void buildAndSendManualModel() {
		...
		List<FooCard2d> topCards2d = new ArrayList<>();
		List<FooCard2d> bottomCards2d = new ArrayList<>();
		
		for(Object2d object2d : getObjects()) {

			if(object2d instanceof FooCard2d) {

				if(object2d.topLeftY() < CommonConstants.INITIAL_CARD_HEIGHT_MILIMETERS) {
					
					topCards2d.add((FooCard2d) object2d);

				}else {

					bottomCards2d.add((FooCard2d) object2d);
				}
			}
		}
		...
		for(int i = 0; i < bottomCards2d.size(); i++) {
			FooCard2d card2d = bottomCards2d.get(i);
		...
		for(FooCard2d card2d : topCards2d) {
			if(card2d.isNullCard()) {
				targetList.add(null);
			}else {
